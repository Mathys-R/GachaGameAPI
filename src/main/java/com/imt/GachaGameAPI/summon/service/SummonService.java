...
public class SummonService {
    ...
    public List<SummonDto> regenerateSummons(List<Summon> pastSummons, String token) {
        List<SummonDto> regeneratedSummons = new ArrayList<>();
    
        for (Summon pastSummon : pastSummons) {
            Summon summon = new Summon(pastSummon.getMonsterId(), pastSummon.getUserId(), pastSummon.getTimestamp());
            summonDao.save(summon);
            ...
            RestTemplate restTemplate = new RestTemplate();
            String url = "http://api-player:8082/player/" + pastSummon.getUserId() + "/add-monster/" + pastSummon.getMonsterId();
            ...
            System.out.println("Added monster " + pastSummon.getMonsterId() + " to player " + pastSummon.getUserId());

            // Ajout à la liste des invocations re-générées
            regeneratedSummons.add(new SummonDto(pastSummon.getMonsterId(), pastSummon.getTimestamp()));
